...
# slitPA=32
# BCGmag = 21.23
# guideRA,guideDEC,guidemag = (184.82564,50.87693,16.62)
# cosdec = np.cos(np.radians(guideDEC))
# aligninfo = [[184.94876,50.91130,16.08], # maybe off SE corner, but great if it's on!
...
Lclust = np.exp(-0.5*(z-zclust)**2/zerr**2)/zerr
#print(Lclust)
...
# special targets.
nspecial = 0
for thisRA,thisDEC,thismag,thispri in specialtargets:
    ...
    outstr = '%-15s %-13s %-12s 2000.0 %5.2f r %d 1 0 %d' %  (name,rastr,decstr,thismag,thispri,slitPA)
    print(outstr)
...
for i in range(len(Lclust)):
    ...
    distfromctr = np.sqrt(((myRA-maskRA)*cosdec)**2+(myDEC-maskDEC)**2)
    if distfromctr < 0.083: # 0.083 deg = 5' = 1 chip
        # check for match w/known z
        distfromknown = np.sqrt(((myRA-knownra)*cosdec)**2+(myDEC-knowndec)**2)
    ...
    priority *= 24-np.clip(mag,BCGmag,None)
    ...
    outstr = '%-15s %-13s %-12s 2000.0 %5.2f r %d 1 0 %d' %  (name,raformatter(myRA),decformatter(myDEC),mag,priority,slitPA)
    color = 'green'   
    print(outstr)
...
# add sky slits
#outstr = '01010101 15:09:00 +57:59:22 2000.0 20.00 r 5000 1 1 %d' %  (slitPA)
#print(outstr)
#outstr = '01010102 15:09:03.28 +57:59:22.4 2000.0 20.00 r 5000 1 1 %d' %  (slitPA)
#print(outstr)
#outstr = '01010103 15:08:52.46 +57:59:31.4 2000.0 20.00 r 5000 1 1 %d' %  (slitPA)
#print(outstr)
#outstr = '01010104 15:08:44.698 +57:57:39.13 2000.0 20.00 r 5000 1 1 %d' %  (slitPA)
#print(outstr)
