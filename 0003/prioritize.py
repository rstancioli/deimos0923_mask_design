...
if len(sys.argv)==2:
    ...
    print(sys.argv)
    sys.stderr.write('Usage: prioritize.py input_file.json \n') 
    sys.exit(1)
...
votb = (votable.parse_single_table(source=v.guide)).to_table()
v.guideRA,v.guideDEC,v.guidemag = list(votb.iterrows('_RAJ2000','_DEJ2000','rmag'))[0]
sys.stderr.write("{} {}".format(v.guideRA, v.guidemag) )

votb2 = (votable.parse_single_table(source=v.aligninfo)).to_table()
...
knowndata = ascii.read('archivalz.csv')
...
outstr = '%-15s %-13s %-12s 2000.0 %5.2f r -1 0 1' %  (name,rastr,decstr,v.guidemag)
print(outstr)
...
for thisRA,thisDEC,thismag,thispri in v.specialtargets:
    ...
    outstr = '%-15s %-13s %-12s 2000.0 %5.2f r %d 1 0 %d' %  (name,rastr,decstr,thismag,thispri,v.slitPA)
    print(outstr)
...
for i in range(len(Lclust)):
    ...
    outstr = '%-15s %-13s %-12s 2000.0 %5.2f r %d 1 0 %d' %  (name,raformatter(myRA),decformatter(myDEC),mag,priority,v.slitPA)
    color = 'green'   
    print(outstr)
